...
@section('content')
    <div class="container-fluid py-5 ">
        {!! Form::open(["action"=>"AdminController@update","method"=>"post", "enctype" => "multipart/form-data","class"=>""]) !!}
        @csrf
        <input type="hidden" name="id" value="{{ $post->id }}">
        <div class="form-group d-flex justify-content-between con">
            <div class="col-6" style="padding-left: 0">
                {{Form::text("title",$post->title,["class"=>"form-control ","placeholder"=>"სათაური"])}}
            </div>
        ...
        <div class="row" style="margin-right: 0px;">
            <div class="form-group col-6">
                ...
                {{Form::textarea("mdescript",$post->miniContent,["class"=>"form-control","placeholder"=>"აღწერა","id"=>"editor"])}}
            </div>
            <div class="form-group col-6">
                {{Form::label("Course discription","სრული აღწერა")}}
                {{Form::textarea("descript",$post->content,["class"=>"form-control","placeholder"=>"აღწერა","id"=>"editor2"])}}
            </div>
        </div>
